...
from devcrew.crew import Devcrew
...
def test():
    ...
    try:
        Devcrew().crew().test(n_iterations=int(sys.argv[1]), eval_llm=sys.argv[2], inputs=inputs)
...
def run_with_trigger():
    ...
    try:
        result = Devcrew().crew().kickoff(inputs=inputs)
        return result
    except Exception as e:
